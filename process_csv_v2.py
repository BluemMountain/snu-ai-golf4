...
# User's provided data from Step Id 844
raw_csv = """
...
# Member sorting
names_row = reader[0]
metadata_cols_count = 3  # count, Date, CC/HD
member_cols = []

for i in range(metadata_cols_count, len(names_row)):
    name = names_row[i].strip()
    if name:
        # Collect all data for this member
        member_data = [row[i] for row in reader]
        member_cols.append((name, member_data))

# Sort members by name
member_cols.sort(key=lambda x: x[0])

# Reconstruct the rows
new_rows = []
for row_idx in range(len(reader)):
    original_row = reader[row_idx]
    new_row = original_row[:metadata_cols_count]
    for name, data in member_cols:
        new_row.append(data[row_idx])
    new_rows.append(new_row)

# Recalculate Row 1 (Averages)
for col_idx in range(metadata_cols_count, len(new_rows[0])):
    scores = []
